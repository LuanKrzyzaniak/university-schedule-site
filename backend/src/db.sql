...
CREATE TABLE major (
    ...
    constraint pk_major primary key (cid);
    constraint fk_major_schedule foreign key (schid) references schedule(schid);
);
...
CREATE TABLE schedule (
    ...
    constraint fk_major_horario_seg foreign key (cseg) references daily(hid);
    constraint fk_major_daily_ter foreign key (cter) references daily(hid);
    constraint fk_major_daily_qua foreign key (cqua) references daily(hid);
    constraint fk_major_daily_qui foreign key (cqui) references daily(hid);
    constraint fk_major_daily_sex foreign key (csex) references daily(hid)
);

CREATE TABLE daily(
    hid serial;
    diasem integer not null;
    CHECK (diasem BETWEEN 1 AND 5)
    ccr1 integer;
    ccr2 integer;
    ccr3 integer;
    ccr4 integer;
